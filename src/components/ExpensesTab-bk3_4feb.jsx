...
export default function ExpensesTab({ group, currentUser }) {
  ...
  const blankForm = useCallback(() => ({
    ...
    equalMembers: [],
    // percentage mode: { [userId]: percentage }
    pctMap      : {},
    // custom $ mode: { [userId]: dollarString }
    customMap   : {}
  }), [currentUser.id]);
  ...
  /* seed maps for the current member list */
  const seedMaps = (mList) => {
    const count = mList.length || 1;
    const fair  = parseFloat((100 / count).toFixed(2));
    const pctM  = {};
    const cusM  = {};
    mList.forEach(m => { pctM[m.id] = fair; cusM[m.id] = ''; });
    return { pctMap: pctM, customMap: cusM, equalMembers: mList.map(m => m.id) };
  };

  /* ── modal open / reset ── */
  const openModal = () => {
    const seeded = seedMaps(members);
    setForm({
      ...blankForm(),
      ...seeded
    });
  ...
  const switchSplitType = (newType) => {
    setForm(prev => {
      // re-seed percentage map based on currently checked equal members
      // so switching from equal → % keeps the same people ticked
      const active = prev.equalMembers.length > 0 ? prev.equalMembers : members.map(m => m.id);
      const count  = active.length || 1;
      const fair   = parseFloat((100 / count).toFixed(2));
      const pctM   = {};
      const cusM   = {};
      members.forEach(m => {
        pctM[m.id]  = active.includes(m.id) ? fair : 0;
        cusM[m.id]  = active.includes(m.id) && prev.amount
                        ? parseFloat((parseFloat(prev.amount) / count).toFixed(2)).toString()
                        : '';
      });
      return { ...prev, splitType: newType, pctMap: pctM, customMap: cusM };
    });
  };

  /* ── EQUAL: toggle a member ── */
  ...
  /* ── PERCENTAGE: toggle a member in/out ── */
  const togglePctMember = (id) => {
    setForm(prev => {
      ...
      const fair        = parseFloat((100 / count).toFixed(2));
      const pctM        = { ...prev.pctMap };
      members.forEach(m => {
        pctM[m.id] = newActive.includes(m.id) ? fair : 0;
      });
      return { ...prev, pctMap: pctM };
    });
  };

  /* ── PERCENTAGE: change one member's % and redistribute remainder evenly ── */
  ...
  /* ── CUSTOM $: toggle a member in/out ── */
  const toggleCustomMember = (id) => {
    setForm(prev => {
      const isActive = prev.customMap[id] !== '' && prev.customMap[id] !== undefined;
      const cusM     = { ...prev.customMap };
      // we use pctMap as the "is active" flag for custom too (percentage > 0 = active)
      const pctM     = { ...prev.pctMap };
      if (isActive) {
        cusM[id] = '';
        pctM[id] = 0;
        ...
        cusM[id] = '';
        pctM[id] = 1; // just marks as active; actual value doesn't matter for custom
      }
      return { ...prev, customMap: cusM, pctMap: pctM };
    });
  };

  /* ── CUSTOM $: change one member's dollar amount ── */
  ...
  const customTotal = Object.values(form.customMap).reduce((s, v) => s + (parseFloat(v) || 0), 0);
  const isCustomValid = totalAmount > 0 && Math.abs(customTotal - totalAmount) < 0.01
    && Object.values(form.customMap).some(v => parseFloat(v) > 0);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (form.splitType === SPLIT_EQUAL) {
        ...
        // CUSTOM $
        payload.customSplits = Object.entries(form.customMap)
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              {form.splitType === SPLIT_CUSTOM && (
                <Field label={
                  ...
                  <div className="border border-gray-200 rounded-lg divide-y divide-gray-100">
                    {members.map(m => {
                      const isActive = (form.pctMap[m.id] || 0) > 0;
                      return (
                        <div key={m.id} className="flex items-center gap-3 px-3 py-2">
                          <label className="flex items-center gap-2 flex-1 cursor-pointer">

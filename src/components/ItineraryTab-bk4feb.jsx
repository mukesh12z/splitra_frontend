...
import { Plus, X, Calendar, Clock, MapPin, Search, Trash2 } from 'lucide-react';
import api from '../services/api';

/* ────────────────────────────────────────────────────
   Nominatim geocode helper  (OpenStreetMap – free, no key)
   Returns [{ label, lat, lng }]  or  [] on error/empty
   ──────────────────────────────────────────────────── */
...
export default function ItineraryTab({ group }) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      /* 1. create the itinerary item (controller already persists lat/lng) */
      ...
      /* 2. if coords exist, also create a Location pin so it shows on MapTab */
      if (form.latitude && form.longitude) {
        await api.post('/locations', {
          groupId   : group.id,
          name      : form.activity,             // pin label = activity name
          address   : form.location,             // full geocode string as address
          latitude  : form.latitude,
          longitude : form.longitude,
          category  : 'itinerary',               // distinguishes auto-created pins
          notes     : form.description || null
        });
      }
  ...
  /* group items by date for day-section headers */
  ...
  return (
    <div className="space-y-6">
      ...
      {sortedDays.map(day => {
        ...
        const d        = new Date(day + 'T12:00:00');          // noon avoids TZ-shift on date display
        return (
          <div key={day}>
            {/* day badge */}
            ...
            {/* items for this day – numbered sequentially within the day */}
                  ...
                  <div className="flex gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex justify-between items-start">
                        ...
                        <button onClick={() => handleDelete(item.id)} className="text-gray-300 hover:text-red-500 ml-2 flex-shrink-0">
                          <Trash2 size={17}/>
                        </button>
                      </div>
          ...
          <div className="bg-white rounded-xl w-full max-w-md max-h-[90vh] overflow-y-auto p-6">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button type="submit"
                  className="flex-1 py-2.5 bg-indigo-600 text-white rounded-lg font-semibold text-sm hover:bg-indigo-700">Add Activity</button>
              </div>
            </form>
          </div>
        </div>
